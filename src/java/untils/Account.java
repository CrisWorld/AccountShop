...
public class Account {
    ...
    private static final String SELECT_ACCOUNT_BY_USERNAME_SQL = "SELECT users.username, users.fullname, users.image, users.role_id, users.cart_id, roles.name AS role_name " +
                                                    "FROM users " +
                                                    "LEFT JOIN roles ON users.role_id = roles.id " +
     ...
     public static User getAccountByUserName(String username){
         ...
         try (Connection connection = Database.getConnect();
            ...
            while (rs.next()) {
                ...
                Integer roleId = rs.getObject("role_id", Integer.class);
                String roleName = rs.getString("role_name");
                account = new User(username, "", fullname, image, true, roleId, cart_id, roleName);
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
